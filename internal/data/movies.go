...
func (m MovieModel) Get(id int64) (*Movie, error) {
	...
	// creating a timeout context, so that a query doesnt take longer than 3s
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	// Using *Movie would make it a nil pointer, and Scan will throw an error
	var movie Movie
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	...
	// after rows.Next finishes, it may have any error, so gotta check for that
	if err = rows.Err(); err != nil {
		return nil, Metadata{}, err
	}

	metadata := caclulateMetadata(totalRecords, filters.Page, filters.PageSize)
